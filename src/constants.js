...
export function classifyGroup(row) {
  // 실제 CSV 헤더: *청약의사, *청약 자격, *구매목적, *나이, *분양 일정 인지
  const 의사=(row['*청약의사']||row['청약의사']||'').trim();
  const 자격=(row['*청약 자격']||row['청약자격']||row['청약 자격']||'').trim();
  const 목적=(row['*구매목적']||row['구매목적']||'').trim();
  const 나이=(row['*나이']||row['나이']||'').trim();
  const 분양=(row['*분양 일정 인지']||row['분양 일정']||'').trim();
...
export function parseCSV(text) {
  ...
  return lines.slice(1).map((line,idx)=>{
    ...
    // 나이
    const age=obj['*나이']||obj['나이']||'-';
    // 성별
    const gender=obj['성별']||'-';
    // 거주 지역: 나의 거주 지역 / 나의거주지역 / 지역
    const region=obj['나의 거주 지역']||obj['나의거주지역']||obj['지역']||'-';
    // 마케팅 수신동의
    const marketing=obj['*마케팅 수신동의']||obj['마케팅 수신동의']||'';
    // 자격/목적/의사
    const 자격=obj['*청약 자격']||obj['청약자격']||obj['청약 자격']||'';
    const 목적=obj['*구매목적']||obj['구매목적']||'';
    const 의사=obj['*청약의사']||obj['청약의사']||'';
