...
export function classifyGroup(row) {
  // 다양한 컬럼명 형태 모두 지원 (띄어쓰기 있음/없음, 별표 있음/없음)
  const 의사=((row['청약의사']||row['*청약의사']||'')).trim();
  const 자격=((row['청약자격']||row['청약 자격']||row['*청약 자격']||row['*청약자격']||'')).trim();
  const 목적=((row['구매목적']||row['*구매목적']||'')).trim();
  const 나이=((row['나이']||row['*나이']||'')).trim();
  const 분양=((row['분양일정']||row['분양 일정']||row['분양 일정 인지']||row['*분양 일정 인지']||'')).trim();
...
export function parseCSV(text) {
  ...
  return lines.slice(1).map((line,idx)=>{
    ...
    // 자격/목적/의사/분양 — 띄어쓰기 있음/없음 모두 인식
    const 자격=obj['청약자격']||obj['청약 자격']||'';
    const 목적=obj['구매목적']||'';
    const 의사=obj['청약의사']||'';
    const 분양=obj['분양일정']||obj['분양 일정']||obj['분양 일정 인지']||'';
    return {
      id:idx+1,
